...
@Component({
  selector: 'app-totalday-table',
  templateUrl: './totalday-table.component.html',
  styleUrls: ['./totalday-table.component.css']
})
export class TotaldayTableComponent  implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.term$.pipe(debounceTime(1000), distinctUntilChanged()), this.paginator.page).subscribe(data=>{
      ...
      if(this.isLoggedIn) {
        ...
        this.tableService!.getDataTotalday(this.sort.active, this.sort.direction, this.paginator.pageIndex,this.paginator.pageSize, 
