...
@Component({
  selector: 'app-leverage-table',
  templateUrl: './leverage-table.component.html',
  styleUrls: ['./leverage-table.component.css']
})
export class LeverageTableComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.term$.pipe(debounceTime(1000), distinctUntilChanged()), this.paginator.page).subscribe(data=>{
      ...
      if(this.isLoggedIn) {
        this.tableService!.getDataLeverage(this.sort.active, this.sort.direction, this.paginator.pageIndex,this.paginator.pageSize, 
