...
@Component({
  selector: 'app-testresult-table',
  templateUrl: './testresult-table.component.html',
  styleUrls: ['./testresult-table.component.css']
})
export class TestresultTableComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    merge(this.sort.sortChange, this.term$.pipe(debounceTime(1000), distinctUntilChanged()), this.paginator.page).subscribe(data=>{
      ...
      if(this.isLoggedIn) {
        ...
        this.tableService!.getDataTestResult(this.sort.active, this.sort.direction, this.paginator.pageIndex,this.paginator.pageSize, 
